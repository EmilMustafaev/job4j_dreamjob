...
class CandidateControllerTest {
    ...
    @Test
    public void whenRequestExistingCandidateByIdThenGetCandidatePageWithCandidateData() {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
        var city1 = new City(1, "Москва");
        var city2 = new City(2, "Санкт-Петербург");
        var expectedCities = List.of(city1, city2);
    ...
    @Test
    public void whenRequestNonExistingCandidateByIdThenGetErrorPageWithMessage() {
        var nonExistingId = 999;
        when(candidateService.findById(nonExistingId)).thenReturn(Optional.empty());

        var model = new ConcurrentModel();
        var view = candidateController.getById(model, nonExistingId);
    ...
    @Test
    public void whenUpdateExistingCandidateThenRedirectToCandidatesPage() throws Exception {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var candidateArgumentCaptor = ArgumentCaptor.forClass(Candidate.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    public void whenUpdateNonExistingVacancyThenGetErrorPageWithMessage() throws Exception {
        var candidate = new Candidate(999, "test1", "desc1", now(), 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
    ...
    public void whenDeleteExistingCandidateThenRedirectToCandidatesPage() {
        var existingId = 1;
        when(candidateService.deleteById(existingId)).thenReturn(true);

        var model = new ConcurrentModel();
        var view = candidateController.delete(model, existingId);
    ...
    public void whenDeleteNonExistingCandidateThenGetErrorPageWithMessage() {
        var nonExistingId = 999;
        when(candidateService.deleteById(nonExistingId)).thenReturn(false);

        var model = new ConcurrentModel();
        var view = candidateController.delete(model, nonExistingId);
        var actualMessage = model.getAttribute("message");
